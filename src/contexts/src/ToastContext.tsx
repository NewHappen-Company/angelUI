...
interface ToastContextData {
  addToast(message: Omit<IToastProps, 'id'>): void;
...
export function ToastContextProvider({ children }: { children: React.ReactNode }) {
  ...
  let wrapper = `${toasts.length ? `toasts-wrapper-${toasts[toasts.length - 1].position}` : 'toasts-wrapper-top-right'}`;
  ...
  const addToast = useCallback(
    function ({
      ...
      setTimeout(() => setToasts((toasts) => toasts.slice(1)), duration);
    },
    [setToasts]
  );
